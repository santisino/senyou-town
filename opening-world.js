...
export class ForestWorld {
 ...
 async load(){
  const decoder=new DRACOLoader().setDecoderPath('./vendor/draco/').setWorkerLimit(2);const loader=new GLTFLoader().setDRACOLoader(decoder);const [g,a]=await Promise.all([loader.loadAsync('./assets/opening-day.glb?v=grove3'),loader.loadAsync('./assets/resident-v2.glb')]);decoder.dispose();
  ...
  this.seedling=this.town.getObjectByName('Seedling');this.orbs=Array.from({length:3},(_,i)=>this.town.getObjectByName('EnergyOrb_'+i));this.can=this.town.getObjectByName('WateringCan');this.drops=Array.from({length:8},(_,i)=>this.town.getObjectByName('WaterDrop_'+i));this.drops.forEach(d=>{if(d)d.visible=false;});this.grovePlants=Array.from({length:3},(_,i)=>this.town.getObjectByName('GrovePlant_'+i));
  this.setGrove({value:0,progress:0,label:'从一棵小树开始'},0,'我们的第一片合种林',false,true);this.setProjects({});this.showTown();return this;
 }
 ...
 refreshPins(){this.clearPins();if(this.mode!=='town'||!this.cabins)return;
  ...
  this.addPin('commons',this.groveName||'我们的第一片合种林',new THREE.Vector3(2.7,1.8,3.3),()=>this.onSelect({type:'commons'}));
  this.addPin('energy',this.introGrove?'合作，让能量生长':this.pendingEnergy?`收取 ${this.pendingEnergy} 份能量`:'我的能量小袋',new THREE.Vector3(-2,3.2,1),()=>this.onSelect({type:'energy'}));
  this.pins.find(p=>p.id==='energy').el.classList.add('energy-pin');
  this.addPin('species','梭梭 · 胡杨 · 花棒',new THREE.Vector3(8,1.5,13),()=>this.onSelect({type:'species'}));
 }
 setEnergy(){this.setGrove({value:0,progress:0,label:'从一棵小树开始'},0,'我们的第一片合种林',false,true);}
 setGrove(state,pending,name,ceremony=false,intro=false){if(!this.tree)return;this.groveName=name;this.pendingEnergy=pending;this.introGrove=intro;this.groveState=state;this.tree.visible=state.value>0;this.seedling.visible=state.value===0;this.targetGrowth=.3+state.progress*.7;this.grovePlants.forEach((p,i)=>{if(p)p.visible=state.progress>=[.2,.55,.9][i];});this.orbs.forEach((o,i)=>{if(o)o.visible=intro||pending>i*5;});this.scene.background.set(ceremony?'#eee3c7':'#e2ead7');this.scene.fog.color.copy(this.scene.background);if(this.mode==='town')this.refreshPins();}
 focusGrove(){this.focus(innerWidth>800?[16,17,22]:[15,18,22],[innerWidth>800?-4:0,1,0]);}
 pour(){this.pourStarted=performance.now();}
 snapshot(){this.renderer.render(this.scene,this.camera);return this.renderer.domElement.toDataURL('image/png');}
 animate(){requestAnimationFrame(()=>this.animate());const now=performance.now(),dt=Math.min(this.clock.getDelta(),.05);if(this.tree){const s=this.reduced?this.targetGrowth:THREE.MathUtils.lerp(this.tree.scale.x,this.targetGrowth,Math.min(1,dt*1.7));this.tree.scale.setScalar(s);this.orbs.forEach((o,i)=>{if(o)o.position.y=2.1+i*.55+(this.reduced?0:Math.sin(now*.0015+i)*.14);});const pouring=!!this.pourStarted&&now-this.pourStarted<2600&&!this.reduced;if(this.can)this.can.rotation.z=pouring?-.4:0;this.drops.forEach((o,i)=>{if(!o)return;o.visible=pouring;if(pouring){const t=((now-this.pourStarted)/750+i/8)%1;o.position.set(-1.6+1.6*t,1.9-1.4*t,1.8-1.8*t);}});}if(this.mode==='town'&&this.avatar?.visible&&this.walkTarget){const delta=this.walkTarget.clone().sub(this.walkPosition);delta.y=0;const distance=delta.length();if(distance<.08){this.walkTarget=null;this.avatar.position.copy(this.walkPosition);}else{delta.normalize();this.walkPosition.addScaledVector(delta,Math.min(distance,dt*3.5));this.avatar.position.copy(this.walkPosition);if(!this.reduced)this.avatar.position.y+=Math.abs(Math.sin(now*.012))*.06;this.avatar.rotation.y=Math.atan2(delta.x,delta.z);}}if(this.move){const f=Math.min(1,(now-this.move.start)/(this.reduced?1:1000)),t=1-(1-f)**3;this.camera.position.lerpVectors(this.move.p,this.move.end,t);this.controls.target.lerpVectors(this.move.t,this.move.aim,t);if(f===1)this.move=null;}this.controls.update();for(const p of this.pins){const v=p.position.clone().project(this.camera);p.el.style.left=(v.x*.5+.5)*this.container.clientWidth+'px';p.el.style.top=(-v.y*.5+.5)*this.container.clientHeight+'px';p.el.hidden=v.z>1||Math.abs(v.x)>1||Math.abs(v.y)>1;}this.renderer.render(this.scene,this.camera);}
}
